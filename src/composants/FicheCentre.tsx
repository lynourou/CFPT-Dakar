...
export const FicheCentre: React.FC<FicheCentreProps> = ({
  ...
  const gererPartage = () => {
    ...
    url.searchParams.set('centre', String(centre.id));
  ...
  const valeurValide = (v: any): boolean => {
    ...
    if (typeof v === 'string' && (v.trim() === '' || v.trim().toLowerCase() === 'null' || v.trim().toLowerCase() === 'undefined')) return false;
    ...
    >
      ...
      >
        ...
        <div className="bg-slate-900 text-white p-5 sm:p-6 relative shrink-0">
            ...
            {valeurValide(centre.type) && (
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                  centre.type === 'Public' ? 'bg-blue-600 text-white' : 'bg-emerald-600 text-white'
                }`}
              >
                {centre.type}
              </span>
          ...
          {/* Nom du centre */}
          <h2 id="fiche-titre-centre" className="text-lg sm:text-xl font-bold tracking-tight text-white leading-tight">
            {centre.nom}
          </h2>

          {/* Nom officiel */}
          {valeurValide(centre.nom_officiel) && centre.nom_officiel !== centre.nom && (
        ...
        {/* Corps déroulant de la fiche */}
        <div className="p-5 sm:p-6 overflow-y-auto space-y-5 text-slate-800">
          {/* Photo réelle de l'établissement (uniquement si renseignée dans la base réelle) */}
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-700 mt-0.5">
                    Cette action supprimera définitivement « {centre.nom} » de la base Firestore.
                  </p>
                </div>
              </div>
          ...
          {valeurValide(centre.description) && (
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
              ...
              <p className="text-xs sm:text-sm text-slate-700 leading-relaxed">{centre.description}</p>
            </div>
          )}

          {/* Formations, Filières et Diplômes */}
          {(valeurValide(centre.formation) || valeurValide(centre.filiere) || valeurValide(centre.diplomes)) && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
                {valeurValide(centre.formation) && (
                  <div className="space-y-0.5">
                    ...
                    <p className="font-semibold text-slate-800">{centre.formation}</p>
                  </div>
                )}
                ...
                {valeurValide(centre.filiere) && (
                  <div className="space-y-0.5">
                    ...
                    <p className="font-semibold text-blue-900">{centre.filiere}</p>
                  </div>
                )}
                ...
                {valeurValide(centre.diplomes) && (
                  <div className="sm:col-span-2 space-y-0.5">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      {centre.diplomes?.split(',').map((diplome, idx) => (
                        <span
                          key={idx}
          ...
          {(valeurValide(centre.adresse) || valeurValide(centre.telephone) || valeurValide(centre.email) || valeurValide(centre.site_web)) && (
            <div className="border border-slate-200 rounded-xl p-4 space-y-3 bg-white shadow-2xs">
              <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
                {valeurValide(centre.adresse) && (
                  <div className="sm:col-span-2 flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <span className="text-slate-800 font-medium">{centre.adresse}</span>
                    </div>
                  </div>
                )}
                ...
                {valeurValide(centre.email) && (
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Courriel</span>
                      <a
                ...
                {valeurValide(centre.site_web) && (
                  <div className="sm:col-span-2 flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Site Internet officiel</span>
                      <a
          ...
          {/* Renseignements complémentaires & SIG */}
          {(valeurValide(centre.capacite) || valeurValide(centre.date_creation) || valeurValide(centre.source)) && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              {valeurValide(centre.source) && (
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                  ...
                  <span className="text-xs font-semibold text-slate-800 block mt-0.5 truncate" title={centre.source}>
                    {centre.source}
                  </span>
                </div>
              )}
          ...
          {valeurValide(centre.latitude) && valeurValide(centre.longitude) && (
            <div className="p-3 rounded-xl bg-slate-900 text-slate-200 text-xs flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center space-x-2">
                ...
                <span>
                  <strong>Coordonnées GPS :</strong> {Number(centre.latitude).toFixed(5)}° N, {Number(centre.longitude).toFixed(5)}° O
                </span>
              </div>
              <span className="text-[10px] text-slate-400 font-mono">Système WGS84 (EPSG:4326)</span>
            </div>
          )}
        </div>

        {/* Barre d'actions en pied de fiche */}
